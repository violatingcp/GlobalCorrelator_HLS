...
int main(int argc, char ** argv) {
  ...
  for(int nevt = 0; nevt < 1028; nevt++)  { 
    ...
    for(int idepth = 0; idepth < DEPTH; idepth++) { 
      for(int i0 = 0; i0 < NTRACK;  i0++) {
	input[idepth][i0] = chparts[idepth][i0];
      }
      for(int i0 = 0; i0 < NEMCALO; i0++) { 
	input[idepth][i0+EMOFFS] = emparts[idepth][i0];
      }
      for(int i0 = 0; i0 < NCALO;   i0++) { 
	input[idepth][i0+HAOFFS] = neparts[idepth][i0];
      }  
      for(int i0 = 0; i0 < NMU;     i0++) { 
	input[idepth][i0+MUOFFS]  = muparts[idepth][i0];
      }
	...
	float pPhi = output[idepth][ipart].range(47,32); pTmp.hwPhi = pPhi; pPhi/=ETAPHI_SCALE;
	int   pId  = output[idepth][ipart].range(63,48); pTmp.hwId  = pId;
