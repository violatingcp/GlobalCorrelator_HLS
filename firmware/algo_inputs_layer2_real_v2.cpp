...
#include "algo_inputs_layer2_real_v2.h"
...
void algo_inputs_layer2_real_v2(hls::stream<axi_t> link_in[NTRACK+NCALO],hls::stream<axi_t> allparts_in [NTRACK*4+NCALO*4]) { 
  #pragma HLS PIPELINE 
  //TM6 = 36
  ...
  PFChargedObj pfne[NREGIONS][NCALO];
  ...
  #pragma HLS ARRAY_RESHAPE variable=pfch  block factor=10   dim=2
  #pragma HLS ARRAY_RESHAPE variable=pfne  block factor=6    dim=2
  ...
  LoopA:
  for(unsigned int idepth =0; idepth < NREGIONS; idepth++) { 
    #pragma HLS PIPELINE II=1
    axi_t chin[NTRACK];
    axi_t nein[NCALO];
    for(int i0 = 0; i0 < NTRACK;  i0++) chin[i0] = link_in[i0].read();
    for(int i0 = 0; i0 < NCALO;   i0++) nein[i0] = link_in[NTRACK+i0].read();
    mp7_unpack_seed    (chin[0],       chseed[idepth]);
    //mp7_unpack_seed    (chin[NTRACK],  chseed[2*idepth+1]);
    //mp7_unpack_seed    (chin[2*NTRACK],chseed[3*idepth+2]);
    mp7_unpack<NTRACK,0>       (chin, pfch[idepth]);
    //mp7_unpack<NTRACK,NTRACK>  (chin, pfch[2*idepth+1]);
    //mp7_unpack<NTRACK,2*NTRACK>(chin, pfch[3*idepth+2]);
    mp7_unpack<NCALO,0>        (nein, pfne[idepth]);
    //mp7_unpack<NCALO,NCALO>    (nein, pfne[2*idepth+1]);
    //mp7_unpack<NCALO,2*NCALO>  (nein, pfne[3*idepth+2]);
  }
  ...
  for(int itau = 0; itau < NTAU; itau++) {
    ...
    for(int iregion = 0; iregion < 4; iregion++) {
      ...
      deltaR_in<NTRACK,NTRACK>(NTRACKOFFSET,  seedeta,seedphi,pfch[pRegion], allparts_in);
      deltaR_in<NCALO,NCALO>  (NSELCALOOFFSET,seedeta,seedphi,pfne[pRegion], allparts_in);
    }
  }
}
